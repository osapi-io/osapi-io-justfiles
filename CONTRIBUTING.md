...
Contributions to osapi-justfiles are very welcome, but we ask that you read
this document before submitting a PR. It covers everything you need:
prerequisites, setup, the conventions recipes follow, and the pull request
workflow.
...
- **Check existing work** — Is there an existing PR? Are there issues
  discussing the change you want to make? Please make sure you
...
- **Backwards compatibility** — Every repository in the organization fetches
  ...
  compatible. If it cannot be, consider opening an issue first so the
  migration can be discussed before you invest your time in a PR.
...
Individual modules require the tools they invoke. `md` needs [uv]; `go` needs
a Go toolchain. Each module's README lists its own requirements.
...
- Use `[group('name')]` attributes to organize recipes (setup, test, fmt,
  lint, dev, build, codegen, docs).
...
- **Prefix every environment variable with `JUST_`**, e.g.
  `JUST_MAIN_PACKAGE`. This is a requirement of the `justfiles` specification,
  not a preference.

Modules consumed by import share one namespace with the consuming justfile, so
prefix their variables with the module name as well — a generic name like
`wrap` silently overwrites any other `wrap` in scope.
...
- **Name the consumers you tested against** — a shared recipe that was never
  run by a real project has not been tested.
